...
RED.view = (function() {
    ...
    function quickAddNode(point, type) {
        quickAddActive = false;
                ...
                if(mouse_mode === RED.state.QUICK_JOINING){
                    ...
                    if(drag_line && drag_line.node){
                        ...
                        if(src.type !== 'rule' && src.type !== 'wildcard'){
                            ...
                            RED.typeSearch.show({
                                type: mousedown_node.type,
                                putType: mousedown_port_type,
                                ...
                                add: quickAddNode.bind(this, point)
                            })
                        }
                    } else {
                        RED.typeSearch.show({
            ...
            if(!mouseover_port){
                ...
                if(mousedown_node.type !== 'rule' && mousedown_port_type !== 'wildcard'){
                    ...
                    RED.typeSearch.show({
                        ...
                        add: function(type){
                            quickAddNode.call(this, point, type)
                            hideDragLines();
                        }
                    })
                }
    ...
    function calculateTextDimensions(str,className,offsetW,offsetH) {
        return [80, 20]
        /* console.log(str,className,offsetW,offsetH);
                    ...
                    if (d.dirty) {
                        ...
                        if (mouse_mode != RED.state.MOVING_ACTIVE) {
                            ...
                            if (d.inputs === 0 && !inputPorts.empty()) {
                                ...
                                var inport = inputGroup.append("rect").attr("class","port").attr("rx",3).attr("ry",3).attr("width",10).attr("height",10)
                                    ...
                                    .on("mouseover",function(d){portMouseOver(d3.select(this),d,PORT_TYPE_INPUT,0);})
                                    .on("mouseout",function(d) {portMouseOut(d3.select(this),d,PORT_TYPE_INPUT,0);});
                                if(RED.nodes.links.some(function(l){
                                        return d.id === l.target.id
                                    })){
                                        inport.classed("port_unuse", false)
                                } else {
                                    inport.classed("port_unuse", true)
                                }
                            }
                            ...
                            var outport = output_group.append("rect").attr("class","port").attr("rx",3).attr("ry",3).attr("width",10).attr("height",10)
                            ...
                            if(RED.nodes.links.some(function(l){
                                    ...
                                    outport.classed("port_unuse", false)
                            } else {
                                outport.classed("port_unuse", true)
                            }
